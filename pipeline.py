...
class Pipeline():
    ...
    def find_similar_models_with_different_classifiers(self):
        ...
        while True:
            ...
            X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=1 / 2, random_state=None)
    ...
    def find_similar_models_with_different_features(self):
        ...
        X = pd.get_dummies(self.data, columns=categorical_features,
                       drop_first=True)
        X_train, X_test, y_train, y_test = train_test_split(X, self.labels, test_size=0.3, random_state=None)
